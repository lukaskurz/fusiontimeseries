"""Extract training runtime and inference speed metrics for different adapter types."""
...
def main():
    """Extract and summarize training metrics by adapter type."""

    # ADAPTERS_TO_EVALUATE = ["Linear", "BilinearLoRA", "RSSBilinearLoRA"]
    # Full context
    ...
    for folder in run_folders:
        ...
        # Skip if not one of the target adapter types
        if method not in ADAPTERS_TO_EVALUATE:
            continue
    ...
    print("\n" + "=" * 70)
    print("Training Runtime by Adapter Type")
    print("=" * 70)
    for adapter in ADAPTERS_TO_EVALUATE:
        if adapter in adapter_train_runtimes:
            mean, error = calculate_mean_and_error(adapter_train_runtimes[adapter])
            print(
                f"{adapter:20s}: {mean:8.2f} ± {error:6.2f} seconds ({len(adapter_train_runtimes[adapter])} runs)"
            )
        else:
            print(f"{adapter:20s}: No data")

    print("\n" + "=" * 70)
    print("Inference Time (1 / eval_samples_per_second) by Adapter Type")
    print("=" * 70)
    for adapter in ADAPTERS_TO_EVALUATE:
        if adapter in adapter_inference_times:
            mean, error = calculate_mean_and_error(adapter_inference_times[adapter])
            print(
                f"{adapter:20s}: {mean:8.6f} ± {error:8.6f} seconds ({len(adapter_inference_times[adapter])} samples)"
            )
        else:
            print(f"{adapter:20s}: No data")

    print("\n" + "=" * 70)
    print(adapter_inference_times)
    print("\n" + "=" * 70)
