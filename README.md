...
- **OOD is finetuning's story alone** (67.94 → 34.10 at k=0); no legit ICL config improves it further. And **the training window matters**: the full-8192-window concat stream is out-of-distribution for a model finetuned on 512-step windows — the win512 cells beat full-window in all 8 paired cells, and under the clamp k=5 ≡ k=10 bit-identically (both reduce to ~the tail of the final example + query).
