...
- **OOD is mostly finetuning's story** (67.94 → 34.10 at k=0); *shape* retrieval (mmr) does not improve it, but **level-aware retrieval does** (Part A, next bullet). And **the 512-window clamp is a context-composition effect, not window-length mismatch**: clamping to the model's training window beats full-window in all 8 mmr cells (under the clamp k=5 ≡ k=10 bit-identically — only the final example's tail + query survive) yet *destroys* the oracle ceiling (9.39 → 19.57 ID) — the clamp is a crude tail-selector that drops the wrong-level example mass legit retrieval inevitably includes, while the oracle's all-level-matched examples benefit from more demonstration mass. Context should contain only matched-tail mass; when retrieval can guarantee that, longer contexts win.
- **Level-aware retrieval on the finetuned model (Part A, 2026-06-13).** Phase 6 used only shape-matching `mmr_euclid`; adding the level-matching `ctx_level` and a level-aware MMR hybrid `mmr_level` (level relevance + shape diversity, `selection.py`) to the grid reproduces the [level-vs-shape split](docs/results/fewshot/level_matching.md) on the finetuned model. **`ctx_level` significantly improves ft OOD** (34.10 → **27.42** at k=5 mean, p_boot=0.000; → **24.58** under the 512 clamp — the best legitimate ft OOD), overturning "no legit ICL config improves OOD" — while `mmr_euclid` (shape) still wins ID (18.62) but leaves OOD ≥ ft k=0, and `mmr_level` dominates neither (OOD 29.57 / ID 34.64). The model-free verdict carries onto the ft model: **shape retrieval for ID, level retrieval for OOD — no single 80-step-context retriever wins both.**
...
#### In-context finetuning (Phase 9): does training on demonstrations beat inherited ICL?

Phase 6's window-clamp analysis showed the finetuned model's in-context ability is *inherited* from base pretraining — it was finetuned on single traces, never on demonstrations. **In-context finetuning (ICF)** trains the same BilinearLoRA recipe ON multi-example ICL concatenations (`finetuning/chronos2/{icl_dataset.py,train_bilinear_icl.py}`, window 2048, k∈{1,3,5} per sample, raw level-clean concatenation, query-only conditioning), with two checkpoints — a **level** model (demos retrieved by context level, train≡test `ctx_level`) and a **random** control — evaluated in `benchmarking/few_shot/run_icl_finetuned.py` → `results/few_shot_v9_icl/`, analyzed in [docs/results/fewshot/icl_finetuning_table.md](docs/results/fewshot/icl_finetuning_table.md) (+ `icl_finetuning_kcurve.png`).

| (mean decoding) | k=0 | best legit ICL | oracle k=10 (ceiling) |
| --------------- | --- | -------------- | --------------------- |
| ICF-level | 59.76 / 35.13 | 30.07 / 29.86 (ctx_level k=5) | 22.20 / **7.49** |
| ICF-random (control) | 46.52 / 64.76 | **16.35** / 57.17 (ctx_level k=10) | 32.80 / 61.82 |
| v6 single-trace ft (8192 win) | 22.20 / 34.10 | 18.62 / 36.00 (mmr_euclid k=5) | 9.39 / 10.89 |

- **ICF makes the model demonstration-dependent** — both checkpoints collapse at k=0 (vs v6 ft's 22.20 ID) and gain massively from ICL (−29 ID, p≤0.012): trained always-with-demos, they are demo-driven where v6 was not.
- **The random control proves the usage is level-specific — the oracle is the smoking gun.** Given perfectly level-matched demos, ICF-level dominates ICF-random: **OOD 7.49 vs 61.82 (Δ−54.3, p_boot=0.000)**, ID 22.20 vs 32.80 (p=0.042); the random model gains ~nothing from oracle demos (it learned to *ignore* demo level). It even beats the v6 oracle ceiling OOD (7.49 vs 10.89, p=0.000) — ICF moved the *ceiling*.
- **But realistic retrieval can't deliver oracle-quality matches, so ICF does not beat inherited ICL**: under `ctx_level` retrieval ICF-level ≈ v6 ft (ID −0.27 n.s., OOD +1.9 n.s.) and the project's best legit ID (15.63, v6 ft @512) is unbeaten. ICF-random's strong 16.35 ID is a level-blind amplitude trick that blows up OOD (57). **The bottleneck is retrieval, not in-context capacity** — closing it needs side information, not more in-context training. (n=6 ID / 5 OOD ⇒ the OOD oracle separation, p_boot=0.000, is the robust headline.)
